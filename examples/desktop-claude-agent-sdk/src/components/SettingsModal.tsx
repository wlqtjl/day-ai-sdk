...
import { IconX, IconEye, IconEyeOff, IconPlugConnected, IconLoader2, IconSearch, IconCheck } from '@tabler/icons-react'
...
export default function SettingsModal({ isOpen, onClose, onSettingsChanged }: SettingsModalProps) {
  ...
  const detectClaudePath = async () => {
    ...
    const currentConfig = await window.dayai.getConfig()
    if (currentConfig.claudeCodePath) {
    ...
    try {
      const detected = await window.dayai.detectClaudePath()
      if (detected) {
        setConfig((prev) => ({ ...prev, claudeCodePath: detected }))
        setClaudeDetected(true)
        // Auto-save the detected path
        await window.dayai.setConfig({ claudeCodePath: detected })
        onSettingsChanged?.()
      }
    } catch {
        ...
        <div className="px-6 py-4 space-y-4">
          ...
          <div>
            ...
            <div className="relative flex gap-2">
              <input
                type="text"
                value={config.claudeCodePath || ''}
                onChange={(e) => {
                  setConfig((prev) => ({ ...prev, claudeCodePath: e.target.value }))
                  setClaudeDetected(false)
                }}
                placeholder="/usr/local/bin/claude"
                className="flex-1 bg-black/20 border border-white/10 rounded-xl px-4 py-2.5 text-white placeholder-white/40 focus:outline-none focus:border-white/30 focus:ring-1 focus:ring-white/20 transition-all font-mono text-sm"
              />
              <button
                type="button"
                onClick={handleDetectClaude}
                disabled={isDetectingClaude}
                className="px-3 py-2.5 rounded-xl bg-black/20 border border-white/10 text-white/60 hover:text-white hover:border-white/30 transition-all disabled:opacity-50 flex items-center gap-1.5"
                title="Auto-detect Claude CLI"
              >
                {isDetectingClaude ? (
                  <IconLoader2 size={18} className="animate-spin" />
                ) : claudeDetected && config.claudeCodePath ? (
            ...
            <p className="mt-1.5 text-xs text-white/40">
              Path to the Claude Code CLI executable. Required for the Claude Agent SDK.
              {claudeDetected && config.claudeCodePath && (
